...
<x-layout :title="$pageTitle" :description="$metaDescription" :robots="$robotsMeta" :canonical="$canonicalUrl">
    ...
    <section class="bg-gradient-to-br from-emerald-50 via-green-50 to-teal-50 py-12 md:py-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-slate-200 p-3 sm:p-4">
                <form action="{{ isset($landing) ? route('jobs.landing', request()->route('slug')) : route('jobs.index') }}" method="GET" class="flex flex-col md:flex-row gap-3">
        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <aside class="lg:w-80 shrink-0">
                <div class="bg-white rounded-2xl border border-slate-200 p-6 sticky top-24">
                    ...
                    <form action="{{ isset($landing) ? route('jobs.landing', request()->route('slug')) : route('jobs.index') }}" method="GET" class="space-y-6" x-data="jobFilters()" @change.debounce.250ms="apply($el)" @submit.prevent="apply($el)">
                        @if($filters->keyword ?? false)
    ...
    {{-- CTA AI Tools Section --}}
    <div class="bg-emerald-50 border-t border-emerald-100 py-10">
        <div class="mx-auto max-w-5xl px-4">
            <p class="text-center text-sm font-semibold text-emerald-700 mb-4">Sebelum melamar, siapkan dirimu dengan tools gratis Lamaraja:</p>
            <div class="flex flex-wrap justify-center gap-3">
                <a href="{{ route('mock-interview.landing') }}" class="inline-flex items-center gap-2 rounded-full bg-white border border-emerald-200 px-4 py-2 text-sm font-semibold text-emerald-800 shadow-sm hover:bg-emerald-100 transition">
                    🎤 Simulasi Interview AI Gratis
                </a>
                <a href="{{ route('cv-matcher.index') }}" class="inline-flex items-center gap-2 rounded-full bg-white border border-emerald-200 px-4 py-2 text-sm font-semibold text-emerald-800 shadow-sm hover:bg-emerald-100 transition">
                    📄 CV Matcher & ATS Checker Gratis
                </a>
                <a href="{{ route('ai-tools.cover-letter') }}" class="inline-flex items-center gap-2 rounded-full bg-white border border-emerald-200 px-4 py-2 text-sm font-semibold text-emerald-800 shadow-sm hover:bg-emerald-100 transition">
                    ✉️ Generator Cover Letter Gratis
                </a>
                <a href="{{ route('ai-tools.skill-gap') }}" class="inline-flex items-center gap-2 rounded-full bg-white border border-emerald-200 px-4 py-2 text-sm font-semibold text-emerald-800 shadow-sm hover:bg-emerald-100 transition">
                    📊 Skill Gap Analyzer Gratis
                </a>
                <a href="{{ route('ai-tools.cv-rewrite') }}" class="inline-flex items-center gap-2 rounded-full bg-white border border-emerald-200 px-4 py-2 text-sm font-semibold text-emerald-800 shadow-sm hover:bg-emerald-100 transition">
                    ✏️ CV Rewrite & ATS Optimizer Gratis
                </a>
            </div>
        </div>
    </div>
