...
<body class="min-h-screen flex flex-col bg-white text-slate-800 antialiased">
    ...
    <header class="glass-header sticky top-0 z-50" x-data="{ mobileMenuOpen: false }">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-14 md:h-16">
                ...
                <nav class="hidden md:flex items-center gap-1" aria-label="Navigasi utama">
                    <a href="{{ route('home') }}" class="nav-pill px-3.5 py-2 rounded-lg text-sm font-medium interactive-focus {{ request()->routeIs('home') ? 'active' : 'text-slate-600 hover:text-slate-900' }}">Beranda</a>
                    <a href="{{ route('jobs.index') }}" class="nav-pill px-3.5 py-2 rounded-lg text-sm font-medium interactive-focus {{ request()->routeIs('jobs.*') ? 'active' : 'text-slate-600 hover:text-slate-900' }}">Cari Loker</a>
                </nav>

                <button @click="mobileMenuOpen = !mobileMenuOpen" class="md:hidden inline-flex items-center justify-center w-10 h-10 rounded-lg text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-colors interactive-focus" :aria-expanded="mobileMenuOpen" aria-label="Toggle menu">
        ...
        <div x-show="mobileMenuOpen" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="md:hidden border-t border-slate-100 bg-white">
            <nav class="px-4 py-2 space-y-1" aria-label="Navigasi mobile">
                <a href="{{ route('home') }}" class="block px-3 py-3 rounded-lg text-sm font-medium min-h-[2.75rem] {{ request()->routeIs('home') ? 'text-teal-600 bg-teal-50' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50' }} transition-colors">Beranda</a>
                <a href="{{ route('jobs.index') }}" class="block px-3 py-3 rounded-lg text-sm font-medium min-h-[2.75rem] {{ request()->routeIs('jobs.*') ? 'text-teal-600 bg-teal-50' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50' }} transition-colors">Cari Loker</a>
            </nav>
        </div>
    </header>
    ...
    {{-- Footer — SEO-rich with internal links --}}
    <footer class="border-t border-slate-100 mt-auto">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-12">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
                ...
                <div>
                    <h3 class="text-sm font-semibold text-slate-900 mb-3">Lokasi</h3>
                    <ul class="space-y-2 text-sm text-slate-500">
                ...
                <div>
                    <h3 class="text-sm font-semibold text-slate-900 mb-3">Jenis Pekerjaan</h3>
                    <ul class="space-y-2 text-sm text-slate-500">
                ...
                <div>
                    <h3 class="text-sm font-semibold text-slate-900 mb-3">Pencarian Populer</h3>
                    <ul class="space-y-2 text-sm text-slate-500">
