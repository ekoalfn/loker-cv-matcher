...
<body class="min-h-screen flex flex-col bg-gradient-to-br from-slate-50 via-slate-100/30 to-white text-slate-800 antialiased">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <a href="{{ route('home') }}" class="flex items-center gap-2.5 font-extrabold text-xl transition-all duration-300 hover:scale-[1.02] group">
                    <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-teal-600 to-cyan-600 flex items-center justify-center shadow-lg shadow-teal-500/20 group-hover:shadow-teal-500/30 transition-shadow">
                ...
                <nav class="hidden md:flex items-center gap-1">
                    <a href="{{ route('home') }}" class="nav-pill px-4 py-2 rounded-xl text-sm font-semibold {{ request()->routeIs('home') ? 'active text-teal-600' : 'text-slate-600 hover:text-teal-600' }}">
                        Beranda
                    </a>
                    <a href="{{ route('jobs.index') }}" class="nav-pill px-4 py-2 rounded-xl text-sm font-semibold {{ request()->routeIs('jobs.*') ? 'active text-teal-600' : 'text-slate-600 hover:text-teal-600' }}">
                        Cari Loker
                    </a>
                </nav>
                ...
                <button
                    ...
                    class="md:hidden inline-flex items-center justify-center w-11 h-11 rounded-xl text-slate-600 hover:text-teal-600 hover:bg-teal-50/50 transition-all duration-200"
    ...
    <footer class="relative mt-auto">
        <div class="absolute inset-0 bg-gradient-to-t from-teal-50/50 to-transparent pointer-events-none"></div>
